...
def feature_normalization(train, test):
    ...
    for i in range(num_features):
        maxes.append(np.max(train[:,i]))
        mins.append(np.min(train[:,i]))
    train_normalized = train
    test_normalized = test
    for i in range(num_features):
        train_normalized[:, i] = (train_normalized[:, i]-mins[i])/(maxes[i]-mins[i])
    for i in range(num_features):
        test_normalized[:, i] = (test_normalized[:, i]-mins[i])/(maxes[i]-mins[i])
